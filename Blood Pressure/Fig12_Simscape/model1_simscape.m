...
function [P, Pa]=model1 (SBP, DBP, fs)
...
a = 0.11; %log(0.1)/(-20); % stiffness coefficient change to 0.075 or 0.11 for very stiff arteries
...
for i=2:length(t)
   ...
   P(i)=P(1)-rate*t(i)+(Va(i)*(P0+760-rate*t(i))/V0);
end
tspan = 0:delta_T:t(end);
%[t2,y] = ode113 (@(t1,P) fopdt(t1,P,P0,dVa_dt, rate, V0,t), tspan, P0);
figure, %plot(t2,y) % Cuff pressure is integrated using two ways: ode113
hold on; plot(t,P) % and another way is manual integration
legend('Cuff pressure vs time')
end
%% 

function dPdt=fopdt(t1,P,P0,dVa_dt, rate, V0,t)
    f = interp1(t,dVa_dt,t1); 
    dPdt=-rate+(f.*(P0+760-rate*t1))/V0;
end
