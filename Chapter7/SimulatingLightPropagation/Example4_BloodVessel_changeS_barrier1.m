...
model.MC.simulationTimeRequested  = 10; % [min] Time duration of the simulation
...
model.MC.wavelength               = wave_l; % [nm] Excitation wavelength, used for determination of optical properties for excitation light
...
M(Z <= zsurf & Y < 0.05 & Y > -0.05 & X < 0.05 & X > -0.05) = 7; % absorber 
...
M(Z > zsurf + epd_thick+derm_thick1+derm_thick2+derm_thick3+derm_thick4) = 8; % dermis1
...
B = B_Cutaneous;% dia 0.2037 %sys 0.2454 %*1.15;%*(1+b1(ii)*0.15);%0.03;%0.002;
S = ii;
W = 0.65;
Me = 0;
musp500 = 42.4;
